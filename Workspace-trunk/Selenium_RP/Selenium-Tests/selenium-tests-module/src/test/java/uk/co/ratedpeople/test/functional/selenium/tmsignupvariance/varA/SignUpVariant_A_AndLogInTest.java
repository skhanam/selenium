...
public class SignUpVariant_A_AndLogInTest extends TestScript {

	private SignUpVariantA signUpVariantA;
	private SignUpVariant_A_AndLogin signUpVariant_A_AndLogin;

	@BeforeClass
	public void beforeClass() throws Exception {
		signUpVariantA = new SignUpVariantA(this.context, scriptUtilities);
		signUpVariant_A_AndLogin = new SignUpVariant_A_AndLogin(this.context, scriptUtilities);
	}
	...
	public void signupVarATest(final String rollOnType, final String tariff, final String screeningType, String tarriffText, String credit) throws Exception {
		String uniqueEmailAccountName = new UniqueEmailAddressGenerator().generateUniqueEmailAddress();
		
		signUpVariantA.signUpVarA(uniqueEmailAccountName, rollOnType, tariff, screeningType);
		signUpVariant_A_AndLogin.login(uniqueEmailAccountName, "password");
		signUpVariant_A_AndLogin.checkPostcode(SignupVarAXpathlocators.dataLookup.get("postcode"));
		signUpVariant_A_AndLogin.checkBilling(tarriffText, credit);
		List<String> skillList = createListSkills();

		signUpVariant_A_AndLogin.checkTrade(skillList);
