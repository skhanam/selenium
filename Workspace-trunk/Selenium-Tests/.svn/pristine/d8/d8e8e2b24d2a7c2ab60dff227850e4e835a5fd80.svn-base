...
public class UpdateExistingBudgetTest extends TestScript {
	...
	public void updateExistingUnsoldJobLead() throws Exception {
		...
		UpdatedJobDetails ujd = updateJob.updateBudget(jobPageUrl, NEW_BUDGET_RANGE);
		assertTrue(ujd.getNewDescription().startsWith("*PRIORITY LEAD* Updated details [and budget]: " + DESCRIPTION));
		assertEquals(ujd.getNewBudget(), NEW_BUDGET_TEXT);
	}
}
